...
```bash
bash scripts/score_fallback.sh 0 /dev/null
```

## Forth: gforth not found

The tokenizer falls back to `scripts/tokenize_fallback.sh` automatically. Install gforth for the real thing:
...
Audit falls back to `scripts/audit_fallback.sh`. Install gnucobol for the native binary:
