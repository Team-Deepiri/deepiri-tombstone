...
DESCRIPTIONS=(
  ...
  ["tokenize/fallback.sh"]="Tokenize prompt (fallback when gforth unavailable)"
  ["score/fallback.sh"]="Score response (fallback when gfortran unavailable)"
  ["audit/fallback.sh"]="Audit record (fallback when gnucobol unavailable)"
  ["request/fallback.sh"]="Build JSON request (fallback when cintsys unavailable)"
