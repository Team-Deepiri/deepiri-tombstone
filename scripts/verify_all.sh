...
# ---- 2. AWK parser ----
header "AWK parser"
echo '{"model":"test","response":"hello world","done":true}' | awk -f src/parse/response.awk > /tmp/dt_test_parse.txt 2>&1
grep -q "hello world" /tmp/dt_test_parse.txt && pass "AWK parse basic" || fail "AWK parse basic"

echo '{"response":"line1\nline2","done":true}' | awk -f src/parse/response.awk > /tmp/dt_test_parse2.txt 2>&1
grep -q "line1" /tmp/dt_test_parse2.txt && pass "AWK parse escape seq" || fail "AWK parse escape seq"

echo '{"error":"not found"}' | awk -f src/parse/response.awk > /tmp/dt_test_parse3.txt 2>&1 && \
  fail "AWK parse missing field" || pass "AWK parse missing field (expected exit)"

# ---- 3. Perl fallback ----
header "Perl http_fallback (syntax)"
perl -c src/transport/http_fallback.pl > /dev/null 2>&1 && pass "Perl syntax OK" || fail "Perl syntax failed"

# ---- 4. Forth tokenize ----
header "Forth tokenize"
echo "hello world" | bash src/tokenize/fallback.sh > /tmp/dt_test_forth.txt 2>&1
...
if command -v gforth >/dev/null 2>&1; then
  echo "hello world" | gforth -e "include src/tokenize/tokenize.fs" > /tmp/dt_test_forth_gforth.txt 2>&1 || true
...
echo "" | bash src/tokenize/fallback.sh > /tmp/dt_test_forth2.txt 2>&1
...
if command -v cobc >/dev/null 2>&1; then
  if cobc -x -o /tmp/dt_audit_test src/audit/ledger.cob 2>/dev/null && \
    ...
    bash src/audit/fallback.sh < /tmp/dt_audit_in.txt > /tmp/dt_audit_out.txt 2>&1
...
if command -v gfortran >/dev/null 2>&1; then
  if gfortran -o /tmp/dt_score_test src/score/score.f 2>/dev/null && \
    ...
    bash src/score/fallback.sh 500 /tmp/dt_response.txt > /tmp/dt_score_out.txt 2>&1
...
bash src/score/fallback.sh 100 /tmp/dt_empty_response.txt > /tmp/dt_score_out2.txt 2>&1
...
bash src/request/fallback.sh llama3.2 "hello" > /tmp/dt_req.json
